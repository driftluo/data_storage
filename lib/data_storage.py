...
class DataStorageBase:
    def __init__(self, database, user, password, port, db, host='localhost'):
        if database == 'mysql':
            self.engine = create_engine('{0}+pymysql://{1}:{2}@{3}:{4}/{5}'.format(database, user, password, host, port, db))
        else:
...
class ExcelDataStorage(DataStorageBase):
    def __init__(self, conf, datas):
        DataStorageBase.__init__(self, conf['database'].lower(), conf['user'], conf['password'], conf['port'], conf['db'],
                                 conf['host'])
        self.datas = datas
        self.conf = conf
